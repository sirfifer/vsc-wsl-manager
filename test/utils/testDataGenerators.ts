 ...
 */
export const distributionGenerators = {
    /**
     * Creates a single WSL distribution with default or custom properties
     */
    createDistribution: (overrides?: Partial<WSLDistribution>): WSLDistribution => ({
        name: 'Ubuntu',
        state: 'Running',
        version: '2',
        default: false,
     ...
     */
    createDistributionList: (): WSLDistribution[] => [
        {
            name: 'Ubuntu',
            state: 'Running',
            version: '2',
     ...
     */
    createRawWSLOutput: (distributions?: WSLDistribution[]): string => {
        const distros = distributions || distributionGenerators.createDistributionList();
        const header = '  NAME                   STATE           VERSION\n';
        const rows = distros.map(d => 
            `${d.default ? '*' : ' '} ${d.name.padEnd(22)} ${d.state.padEnd(15)} ${d.version}`
        ).join('\n');
        return header + rows;
    }
};
...
export const pathGenerators = {
     ...
     */
    createPath: (type: 'tar' | 'dir' | 'config', platform: 'win' | 'unix' = 'win'): string => {
        const bases = {
     ...
     */
    createMaliciousPath: (type: 'traversal' | 'injection' | 'unicode'): string => {
...
export const inputGenerators = {
     ...
     */
    createValidDistributionName: (): string[] => [
        'ubuntu-dev',
        'test_env',
        'MyDistro123',
        'a',  // Single character
     ...
     */
    createInvalidDistributionName: (): Array<{name: string, reason: string}> => [
        { name: '', reason: 'empty' },
     ...
     */
    createCommandInjectionAttempts: (): string[] => [
        '"; echo "hacked',
        '\' && echo \'hacked',
        '`echo hacked`',
        '$(echo hacked)',
...
export const errorGenerators = {
     ...
     */
    createError: (type: 'wsl-not-found' | 'permission' | 'network' | 'timeout'): Error => {
        const errors = {
     ...
     */
    createProcessError: (code: number, signal?: string): any => ({
        code,
        signal,
...
export const responseGenerators = {
     ...
     */
    createVSCodeResponse: (type: 'inputBox' | 'quickPick' | 'dialog', value?: any) => {
        const responses = {
...
export const timingUtils = {
     ...
     */
    createHangingPromise: <T>(): Promise<T> => 
...
export const assertionHelpers = {
     ...
     */
    assertRejects: async (promise: Promise<any>, errorMessage?: string | RegExp) => {
        try {
            await promise;
     ...
     */
    assertCalledWith: (mockFn: jest.Mock, ...expectedArgs: any[]) => {
