...
describe('WSLTreeDataProvider', () => {
    ...
    beforeEach(() => {
        ...
        vscode.EventEmitter.mockImplementation(() => mockEventEmitter);
        // Create tree data provider instance
        treeDataProvider = new wslTreeDataProvider_1.WSLTreeDataProvider(mockWslManager);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('getTreeItem', () => {
        it('should return the same tree item passed to it', () => {
            const mockDistribution = testDataGenerators_1.distributionGenerators.createDistribution();
            const treeItem = new vscode.TreeItem(mockDistribution.name, vscode.TreeItemCollapsibleState.Collapsed);
    ...
    describe('TreeItem properties', () => {
        it('should set correct icon for running distribution', async () => {
            ...
            expect(treeItem.iconPath.id).toBe('vm-active');
            expect(treeItem.iconPath.color?.id).toBe('charts.green');
        });
        it('should set correct icon for stopped distribution', async () => {
            ...
            const treeItem = children[0];
            expect(treeItem.iconPath).toBeInstanceOf(vscode.ThemeIcon);
            expect(treeItem.iconPath.id).toBe('vm');
            expect(treeItem.iconPath.color?.id).toBe('charts.gray');
        });
        it('should set info icon for detail items', async () => {
            ...
            expect(infoItem.iconPath.id).toBe('info');
        });
    ...
    describe('Event handling', () => {
        ...
        it('should update tree when distributions change', async () => {
            ...
            mockWslManager.listDistributions.mockResolvedValue([
                testDataGenerators_1.distributionGenerators.createDistribution({ name: 'Ubuntu' }),
                testDataGenerators_1.distributionGenerators.createDistribution({ name: 'Debian' })
            ]);
    ...
    describe('Error scenarios', () => {
        it('should handle getDistributionInfo errors gracefully', async () => {
            const mockDistribution = testDataGenerators_1.distributionGenerators.createDistribution();
            mockWslManager.getDistributionInfo.mockRejectedValue(new Error('Failed to get info'));
            const parentItem = {
                label: mockDistribution.name,
                distribution: mockDistribution,
                collapsibleState: vscode.TreeItemCollapsibleState.Collapsed,
...
//# sourceMappingURL=wslTreeDataProvider.test.js.map