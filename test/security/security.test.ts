...
describe('Security Tests', () => {
    ...
    describe('Command Injection Prevention', () => {
        ...
        it('should reject all dangerous distribution names', async () => {
            for (const dangerous of dangerousInputs) {
                await expect(
                    wslManager.createDistribution(dangerous, 'Ubuntu')
                ).rejects.toThrow(/Invalid distribution name/);
                
                await expect(
                    wslManager.importDistribution(dangerous, '/path/to/file.tar')
                ).rejects.toThrow(/Invalid distribution name/);
                
                await expect(
                    wslManager.unregisterDistribution(dangerous)
                ).rejects.toThrow(/Invalid distribution name/);
            }
        });
        ...
        it('should reject dangerous file paths', async () => {
            ...
            for (const dangerous of pathAttacks) {
                await expect(
                    wslManager.importDistribution('test', dangerous)
                ).rejects.toThrow(/Invalid TAR file path/);
                
                await expect(
                    wslManager.exportDistribution('test', dangerous)
    ...
    describe('Path Traversal Prevention', () => {
        it('should prevent path traversal in import paths', async () => {
            ...
            for (const attempt of traversalAttempts) {
                const validation = InputValidator.validateFilePath(attempt.input, {
        ...
        it('should normalize paths to prevent tricks', () => {
            ...
            pathTricks.forEach(trick => {
                const validation = InputValidator.validateFilePath(trick);
    ...
    describe('Rate Limiting', () => {
        it('should enforce rate limits to prevent abuse', async () => {
            ...
            const failures = results.filter(r => 
                typeof r === 'string' && r.includes('Rate limit exceeded')
            );
        ...
        it('should have different limits for different operations', async () => {
            ...
            for (let i = 0; i < 15; i++) {
                createPromises.push(
                    wslManager.createDistribution(`test${i}`, 'Ubuntu')
                        .catch(err => err.message)
                );
            }
            
            const createResults = await Promise.all(createPromises);
            const createFailures = createResults.filter(r => 
                typeof r === 'string' && r.includes('Rate limit exceeded')
            );
    ...
    describe('Input Sanitization', () => {
        it('should sanitize display output', () => {
            ...
            maliciousOutputs.forEach(output => {
                const sanitized = InputValidator.sanitizeForDisplay(output);
        ...
        it('should truncate extremely long inputs', () => {
            ...
            const sanitized = InputValidator.sanitizeForDisplay(longInput, 100);
    ...
    describe('Permission Checks', () => {
        it('should check permissions for destructive operations', async () => {
            ...
            await expect(
                wslManager.unregisterDistribution('test')
    ...
    describe('Null Byte Injection', () => {
        it('should prevent null byte injection attacks', async () => {
            ...
            for (const attack of nullByteAttacks) {
                const validation = InputValidator.validateFilePath(attack);
                expect(validation.isValid).toBe(false);
    ...
    describe('Integration Security', () => {
        it('should handle combined attack vectors', async () => {
            ...
            for (const attack of combinedAttacks) {
                ...
                const nameValidation = InputValidator.validateDistributionName(attack);
                expect(nameValidation.isValid).toBe(false);
                
                const pathValidation = InputValidator.validateFilePath(attack);
